...
app.HeatmapView = Backbone.View.extend({
  ...
  toggleColor: function(event) {
    var palName = event.target.dataset.action;
    switch (palName) {
      case "gray": colors = "Greys"; buildScale(); changeParameter(); break;
      case "YlGnBu": colors = "YlGnBu"; buildScale(); changeParameter(); break;
      case "YlOrBr": colors = "YlOrBr"; buildScale(); changeParameter(); break;
    }
  },
  ...
  changeView: function(event) {
    ...
    if (action=="stat") {
      ...
      _.invoke([app.boxchart, app.boxchartlabels, app.boxAxis], "style", "visibility", "visible");
      _.invoke([app.heatmap, app.horAxesA, app.horAxesB, app.legend], "style", "visibility", "hidden");
    };
    if (action=="emp") {
      heatmapView.viewMode = "emp";
      _.invoke([app.boxchart, app.boxchartlabels, app.boxAxis], "style", "visibility", "hidden");
  ...
  markerSwitch: function (event) {
    ...
    app.activeMarkers[this.currentParameter] ^= true;
    if (this.viewMode=="diff") {buildDiffMap(app.prototype_ids[0], app.prototype_ids[1]);}
    buildSumSet(app.activeMarkers[this.currentParameter]);
    buildScale();
    changeParameter();
  },
  ...
  render: function (el) {
    ...
    $.when.apply($, complete).done(function () {
      ...
      thisView.sumset = Array.apply(null, new Array(app.passes.where({prototype_id: app.prototype_ids[0]}).length)).map(Number.prototype.valueOf,0);
      _.each(codeArr, function(mrk) {
        app.activeMarkers[mrk] = 0;
      });   
    });
  }
});
